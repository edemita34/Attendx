...
export const DepartmentsView: React.FC<DepartmentsViewProps> = ({
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!formData.code.trim()) {
      setFormError('Department short code is required.');
      return;
    }
  ...
  const handleDelete = (dept: Department) => {
    ...
    if (!result.success) {
      alert(result.message || 'Cannot delete department.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Department Architecture</h2>
          ...
          return (
            ...
            >
              ...
              <div className="p-5 flex-1 space-y-4">
                ...
                {/* Manager Name */}
                <div className="text-xs text-slate-600 dark:text-slate-300 pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-slate-400">Head / Manager:</span>
                  <span className="font-semibold text-slate-800 dark:text-slate-200">{dept.managerName || 'Arthur Pendelton'}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 text-xs">
                  ...
                  <div className="bg-emerald-50/60 dark:bg-emerald-950/40 p-2.5 rounded-xl border border-emerald-100 dark:border-emerald-800">
                    <span className="text-[11px] text-emerald-700 dark:text-emerald-400 block">Present Today</span>
              ...
              <div className="px-5 py-3 bg-slate-50/80 dark:bg-slate-800/60 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-semibold inline-flex items-center gap-1 transition-colors"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. RND"
                    className="w-full px-3 py-2 text-xs font-mono rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-hidden focus:ring-2 focus:ring-indigo-500 uppercase"
                  />
                </div>
                  ...
                  {COLOR_PALETTE.map((c) => (
                    <button
                      ...
                      className={`w-7 h-7 rounded-lg transition-transform ${
                        formData.color === c ? 'scale-110 ring-2 ring-indigo-500 ring-offset-2 dark:ring-offset-slate-900' : 'opacity-80 hover:opacity-100'
                      }`}
